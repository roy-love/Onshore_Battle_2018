...
class Factory(IStructure):
    ...
    def run(self):
        ...
        if not self.mission is None:
            ...
            if self.mission.action == Missions.TrainBot:
                print ("Factory [{}] TRAIN_BOT {} {}".format(self.unit.id, self.is_working, self.unit.is_factory_producing()))
    ...
    def try_produce_robot(self, unit_type):
        ...
        if self.unit.is_factory_producing():
            ...
            print("Factory [{}] cannot produce the [{}]. Current Karbonite: {}".format(self.unit.id, unit_type, self.game_controller.karbonite()))
        #    return False
