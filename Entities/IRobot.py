...
class IRobot:
    ...
    def update_path_to_target(self):
        ...
        if self.target_location is not None and (self.path is None or len(self.path) == 0):
            print("Creating new path from {},{} to {},{}".format(self.unit.location.map_location().x, self.unit.location.map_location().y, self.target_location.x, self.target_location.y))
            self.path = self.pathfinding_controller.FindPathTo(self.unit.location.map_location(), self.target_location)
...
# Default Missions
    ...
    def one_random_movement(self):
        ...
        if self.game_controller.is_move_ready(self.unit.id) and self.game_controller.can_move(self.unit.id, direction):
            self.game_controller.move_robot(self.unit.id, direction)
