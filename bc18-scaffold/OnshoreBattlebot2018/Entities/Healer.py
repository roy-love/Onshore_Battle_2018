...
class Healer(IRobot):
    ...
    def tryHeal(self, targetUnitId):
        ...
        if not self.game_controller.can_heal(self.unit.id, targetUnitId):
            print ("Healer [{}] cannot heal the target [{}]".format(self.unit.id, targetUnitId))
            return False

        self.game_controller.heal(self.unit.id, targetUnitId)
        return True
    ...
    def tryOvercharge(self, targetUnitId):
        ...
        if not self.game_controller.can_overcharge(self.unit.id, targetUnitId):
            print("Healer [{}] cannot Overcharge the target [{}]".format(self.unit.id, targetUnitId))
            return False

        self.game_controller.overcharge(self.unit.id, targetUnitId)
        return True
	...
	def run(self):
		...
		if not self.mission is None:
			...
			#Heal nearby units
			nearbyAllies = self.game_controller.sense_nearby_units_by_team(self.unit.location.map_location(), 30, self.game_controller.team())
			#TODO Heal nearest ally with lowest health
			for ally in nearbyAllies:
				if (ally.max_health - ally.health) > 15:
					if self.try_heal(ally.id):
						print('Healer {} healed ally {}'.format(self.unit.id, ally.id))
						break

			#Use overcharge on nearby unit
			#TODO Overcharge nearest ally with highest ability cooldown
			for ally in nearbyAllies:
				if not ally.unit_type == bc.UnitType.Healer:
					if self.try_overcharge(ally.id):
						print('Healer {} overcharged ally {}'.format(self.unit.id, ally.id))
						break
	...
	def try_overcharge(self, target_unit_id):
		...
		#Check Overcharge research
		if not self.unit.is_ability_unlocked():
			print("Overcharge not unlocked yet.")
			return False
		...
		if not self.game_controller.can_overcharge(self.unit.id, target_unit_id):
			print("Healer [{}] cannot Overcharge the target [{}]" \
			.format(self.unit.id, target_unit_id))
			return False
