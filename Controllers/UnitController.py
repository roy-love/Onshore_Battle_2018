...
class UnitController:
    ...
    def __delete_killed_units(self):
        ...
        self.robots = [robot for robot in self.robots if any(unit.id == robot.unit.id for unit in self.game_controller.my_units())]
    ...
    def __RegisterUnit(self, unit):
        if unit.unit_type == bc.UnitType.Healer:
            self.robots.append(Healer(self.game_controller, self, self.pathfinding_controller, self.mission_controller, unit))
        elif unit.unit_type == bc.UnitType.Knight:
            self.robots.append(Knight(self.game_controller, self, self.pathfinding_controller, self.mission_controller, unit))
        elif unit.unit_type == bc.UnitType.Mage:
            self.robots.append(Mage(self.game_controller, self, self.pathfinding_controller, self.mission_controller, unit))
        elif unit.unit_type == bc.UnitType.Ranger:
            self.robots.append(Ranger(self.game_controller, self, self.pathfinding_controller, self.mission_controller, unit))
        elif unit.unit_type == bc.UnitType.Worker:
            self.robots.append(Worker(self.game_controller, self, self.pathfinding_controller, self.mission_controller, unit))
